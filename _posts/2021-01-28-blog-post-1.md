...
```
```python
plt.scatter(range(len(results.fittedvalues)),Y,label='actual')
plt.scatter(range(len(results.fittedvalues)),results.fittedvalues,c="r",marker='*',label="fitted",s=10**2)
plt.legend()
plt.show()
```
...
dataDF_C = sm.add_constant(dataDF)
model = ols('Y ~ X1 + X2 + X3 + X4', data=dataDF_C).fit()
```
Results:
```
                            OLS Regression Results                            
